...
class C3_Operator_Model_Exporter:
    ...
    def exportC3UsingRef(outputPath, refPath):
        ...
        while True:
            ...
            if meshName == "v_armet":
                isGarment = True
            if (meshName in PHY_section_names) or (not isGarment and Magic2 == "PHY"):   # if not garemnt, then suppose it has one PHY, just replace it.
    ...
    @staticmethod
    def exportC3WithCustomArm(outputPath, refPath):
        isGarment = False
        scene = bpy.context.scene
        ...
        while True:
            ...
            if (meshName in PHY_section_names) or (not isGarment and Magic2 != "MOT"):   # if not garemnt, then suppose it has one PHY, just replace it.
                ...
                print('Using reference to approximate vertices properties')
                for a in range(vertCount):
                    Unknown1_w_array[original_vertices[a].index] = 0 #Unknown1_array[index]
                    
                    groupName = obj.vertex_groups[original_vertices[a].groups[0].group].name
                    C3GroupIndex = int(groupName.split('_')[-1])
                    Unknown2_w_array[original_vertices[a].index] = C3GroupIndex #Unknown2_array[index]
                    
                    groupName = obj.vertex_groups[original_vertices[a].groups[1].group].name
                    C3GroupIndex = int(groupName.split('_')[-1])
                    Unknown3_w_array[original_vertices[a].index] = C3GroupIndex #Unknown3_array[index]
                    
                    Unknown4_w_array[original_vertices[a].index] = original_vertices[a].groups[0].weight #Unknown4_array[index]
                    Unknown5_w_array[original_vertices[a].index] = original_vertices[a].groups[1].weight #Unknown5_array[index]
...
class C3OP_Save_C3_With_Armature_UI(Operator, ExportHelper):

    bl_idname = "c3op.save_c3_with_armature_ui"
    bl_label = "Save C3 With Armature"
    ...
    def execute(self, context):
        ...
        C3_Operator_Model_Exporter.output_path = self.filepath
        C3_Operator_Model_Exporter.exportC3WithCustomArm(C3_Operator_Model_Exporter.output_path, C3_Operator_Model_Importer.c3_path)
